...
Set the times of the emails with the current timezone of the user instead of
UTC
...
A directory can be attached via :'<,'>Mail but it gets compressed by default to
tar.gz.

Since the command to compress could take some time, the text of

    attach: /path/to/directory.tar.gz

should immediately be added to the buffer so its not blocking the user and the
compression will run in the background. The user will be notified that the
compression has started for the file via diagnostics.

---

Diagnostic warning explaining that attach:ed file does not exist

The warning diagnostic renders a red underline on attach: lines whose file
does not exist, covering the desire for red highlighting of missing attach:
files.
...
refactor the tests so that all that everytime an executable command fails
the notification to the user should be in the following format:

    vimalaya command failed:
    ```sh
    <executable command>
    ```stdout
    <executable command stdout>
    ```
    ```stderr
    <executable command stderr>
    ```

---

Diagnostics for attached file not existing does not underline the filename. It
only underlines the a of attach
...
vimalaya command failed:
```sh
himalaya message compose --from anakin.childerhose@savoirfairelinux.com --to anakin@childerhose.ca --subject sdlfkhfsdkj --attach /home/kin/src/vimalaya.tar.gz --send
```
```stdout
Error: received corrupt message of type InvalidContentType
...
Once scanning for viruses is finished it should look like this:
    ...
    attachments:
      filename: vimalaya.tar.gz /pwd/vimalaya.tar.gz
        mime: application/gzip
        size: 1432415
